...
function sizeLabel(bytes) { if (!Number.isFinite(bytes)||bytes<=0) return 'Unknown size'; const units=['B','KB','MB','GB']; let n=bytes,i=0; while(n>=1024&&i<units.length-1){n/=1024;i++;} return `${n.toFixed(n>=10||i===0?0:1)} ${units[i]}`; }
...
function extractorArgs(client) { return { extractorArgs: `youtube:player_client=${client}` }; }

async function getVideoInfo(url) {
  const clients = ['web_safari', 'android_vr'];
  let lastError;
  for (const client of clients) {
    try {
      return await ytDlp(url, { ...youtubeOptions, ...extractorArgs(client), dumpSingleJson: true, skipDownload: true });
    } catch (error) {
      lastError = error;
      console.warn(`YouTube ${client} extractor failed:`, error?.stderr || error?.message || error);
    }
  }
  throw lastError;
}
...
app.post('/api/analyze', async (req,res)=>{
  ...
  } catch(error) { console.error('YouTube analyze failed:', error?.stderr || error?.stdout || error?.message || error); res.status(422).json({error:'YouTube could not be analyzed right now. Please try again in a moment or try another video.'}); }
});
...
app.post('/api/download',async(req,res)=>{
  ...
  try {
    ...
    let downloaded=false,lastError;
    for (const client of ['web_safari','android_vr']) {
      try { await ytDlp(url,{...options,...extractorArgs(client)}); downloaded=true; break; }
      catch(error){ lastError=error; console.warn(`YouTube ${client} download failed:`,error?.stderr||error?.message||error); }
    }
  ...
  } catch(error){console.error('YouTube download failed:',error?.stderr||error?.stdout||error?.message||error);await fs.rm(tempDir,{recursive:true,force:true}).catch(()=>{});res.status(500).json({error:'Download failed. YouTube may be temporarily restricting this request. Try again or choose another format.'});}
});
...
app.listen(PORT,'0.0.0.0',()=>console.log(`YT Downloader running on port ${PORT}`));
