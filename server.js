...
function validYouTubeUrl(value) {
  try {
    ...
    return ['youtube.com','www.youtube.com','m.youtube.com','youtu.be','www.youtube-nocookie.com'].includes(url.hostname.toLowerCase());
  } catch { return false; }
}
...
const youtubeOptions = {
  ...
  addHeader: ['referer:youtube.com', 'user-agent:Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/131.0 Safari/537.36']
};

async function getVideoInfo(url) {
  try {
    return await ytDlp(url, { ...youtubeOptions, dumpSingleJson: true, skipDownload: true });
  } catch (firstError) {
    console.warn('Default YouTube extractor failed, retrying with android_vr client:', firstError?.stderr || firstError?.message || firstError);
...
app.post('/api/analyze', async (req,res)=>{
  ...
  try {
    ...
    const formats=(info.formats||[])
      ...
      .map(f=>({id:f.format_id,ext:f.ext||'mp4',resolution:f.resolution||(f.height?`${f.height}p`:'Audio'),height:f.height||0,fps:f.fps||null,filesize:f.filesize||f.filesize_approx||0,size:sizeLabel(f.filesize||f.filesize_approx||0),type:f.vcodec!=='none'?'video':'audio',hasAudio:f.acodec!=='none',hasVideo:f.vcodec!=='none',abr:f.abr||null,tbr:f.tbr||null}))
      .filter(f=>f.type==='audio'||f.height>=144)
    ...
    console.error('YouTube analyze failed:', error?.stderr || error?.stdout || error?.message || error);
    res.status(422).json({error:'YouTube could not be analyzed right now. Please try again in a moment or try another video.'});
  }
});
...
app.post('/api/download',async(req,res)=>{
  ...
  try {
    const options={...youtubeOptions,output:outputTemplate};
    if(mode==='audio'){
      options.format=format==='best'?'bestaudio/best':format;
      options.extractAudio=true; options.audioFormat='mp3'; options.audioQuality='0';
      if(ffmpegPath) options.ffmpegLocation=ffmpegPath;
    } else {
      options.format=format==='best'?'bv*[ext=mp4]+ba[ext=m4a]/b[ext=mp4]/b':format;
      options.mergeOutputFormat='mp4';
      if(ffmpegPath) options.ffmpegLocation=ffmpegPath;
    }
    try { await ytDlp(url,options); }
    catch(firstError) {
      console.warn('Default download failed, retrying with android_vr client:', firstError?.stderr || firstError?.message || firstError);
    ...
    console.error('YouTube download failed:', error?.stderr || error?.stdout || error?.message || error);
    await fs.rm(tempDir,{recursive:true,force:true}).catch(()=>{});
    res.status(500).json({error:'Download failed. YouTube may be temporarily restricting this request. Try again or choose another format.'});
  }
});
